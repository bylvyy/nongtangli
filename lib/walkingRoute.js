...
// 步行规划。优先用 D1 里预存的轨迹 (route.walkingPath, 由调用方传入);
// 没有时回退到运行时调高德 / 浏览器侧缓存。
//
// 高德返回的是 GCJ-02 [lat, lng],D1 存的也是 GCJ-02,直接画即可。
...
// 同步返回:D1 预存命中或本地缓存命中。MapInner 会先调它,没结果再调 async 版本。
export function getWalkingPathSync(stops, prebuiltPath) {
  if (prebuiltPath && prebuiltPath.length) return prebuiltPath;
...
export async function getWalkingPath(stops, prebuiltPath) {
  if (!stops || stops.length < 2) return null;
