...
V1 阶段每条路线显示的是 SVG "街区印象图"——主题相关的剪影插画 + 主色渐变。这是诚实的占位:不假装是实景。

撰稿人拍到真实封面图后,**两步替换:**

1. 把图放到 `public/photos/` 下(命名随意,推荐 `{routeId}.jpg`)
2. 在 `lib/routes.js` 对应路线对象里加一行:
   ```js
   coverPhoto: "/photos/wukang-wuyuan.jpg",
   ```

加上后该路线列表卡和详情页顶部都会自动用实拍图代替 SVG。没加的路线继续用 SVG,可逐条迁移。

点位内的 `stop.photos: []` 数组也是同样的方式——填入 URL 后会在点位故事下方横向滚动展示。
